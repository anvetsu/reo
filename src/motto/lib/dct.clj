;; Source: https://github.com/ray1729/thinkstats
(ns motto.lib.dct
...
(defn dct
  "Read Stata data set, return a columnar table."
  [dict-path data-path]
  (let [dict   (read-dict-defn dict-path)
        header (map (comp keyword :name) dict)]
    (with-open [r (reader data-path)]
      (loop [dat (doall (read-dct-data dict r))
             rs (into [] (repeat (count header) []))]
        (if (seq dat)
          (recur (rest dat) (u/spread rs (first dat)))
          (tab/mkt (map #(symbol (name %)) header) rs))))))
